...
async function placeBet(req, res) {
  ...
  try {
    ...
    if (!gameId) {
      return res
        .status(400)
        .json({ success: false, message: "VALIDATION_ERR: GAME_ID_MISSING" });
    }
    ...
    if (!game || !game.is_active) {
      return res
        .status(400)
        .json({ success: false, message: "Game not found or inactive" });
    }
    ...
    // Check mid-time restriction
    let maxBetAmount = null;
    if (game.mid_time && currentTimeStr >= game.mid_time) {
      maxBetAmount = parseFloat(game.max_bet_after_mid_time) || 100.0;
    }
    ...
    // Mode C: Multiple Numbers / Copy-Paste / Haruf
      ...
      for (const num of numbers) {
        ...
        if (actualBetType.includes("haruf") && !isValidHarufNumber(num))
          continue;
      ...
      return res
        .status(400)
        .json({ success: false, message: "Invalid bet format" });
    }

    if (betsToPlace.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "No valid bets identified" });
    }

    // 4. Validate max bet amount if mid-time has passed
    if (maxBetAmount !== null) {
      const invalidBets = betsToPlace.filter(
        (bet) => parseFloat(bet.betAmount) > maxBetAmount,
      );
      if (invalidBets.length > 0) {
        return res.status(400).json({
          success: false,
          message: `Maximum bet amount is ₹${maxBetAmount} after ${game.mid_time}. Please reduce your bet amount.`,
        });
      }
    }

    // 5. PRE-CALCULATION & GATEKEEPER
    const totalAmount = calculateTotalAmount(betsToPlace);
    console.log(
      `DEBUG: placing ${betsToPlace.length} bets, total amount: ${totalAmount}`,
    );
...
async function getBetHistory(req, res) {
  try {
    ...
    const bets = await Bet.getUserBets(
      userId,
      parseInt(limit),
      parseInt(offset),
    );
